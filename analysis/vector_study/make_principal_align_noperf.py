...
top-10% PCs (left axis), yet accuracy fails to climb (right axis, stuck
...
Left  (real-by-construction): energy fraction in the top-10% activation PCs.
...
ALIGN_COLOR = "#2c6fbb"        # top-10% 主子空间能量占比
PERF_COLOR = "#c0392b"         # 准确率（上不去，用红色强调）

# ===== 直接写死的值 (v0..v8) =====
# 左轴：principal 模式下 top-10% 主成分能量占比 (%)。
TOP10PCT = {
...
def main():
    ...
    for ax, dom in zip(axes, ORDER):
        ...
        ax.set_ylim(0, e_top10.max() * 1.25)
        ...
        ax2.text(K - 0.75, b + 0.25, "Base model", color="#555555",
