...
# ============================================================================
# OFFLINE teacher-rollout reverse-KL distillation for Qwen3-4B on math (DeepMath-103K).
#
# Trains the student (Qwen3-4B) to match a frozen teacher (Qwen3-4B-Non-Thinking-RL-Math-Step500)
# on PRE-GENERATED teacher rollouts (teacher_sft_all.parquet from gen_teacher_rollouts_4b.sh).
...
#
# Three modes via the wrappers (4b_distill_offline_{fullparam,lora8,single}.sh):
...
PROJECT_NAME=${PROJECT_NAME:-Qwen3_4B_Math_OfflineDistill}
EXP_NAME_PREFIX=${EXP_NAME_PREFIX:-Qwen3-4B-Math-OfflineDistill}

NNODES=${NNODES:-1}
NGPUS_PER_NODE=${NGPUS_PER_NODE:-8}

# Student (Qwen3-4B base) + teacher to distill from (Qwen3-4B-Non-Thinking-RL-Math = ref model).
MODEL_PATH=${MODEL_PATH:-"${OPV_LEGACY_MODEL_ROOT}/Qwen3-4B"}
TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-"${OPV_LEGACY_MODEL_ROOT}/Qwen3-4B-Non-Thinking-RL-Math-Step500"}

# Offline teacher rollouts (prompt + response), produced by gen_teacher_rollouts_4b.sh.
# Doubles as the TRAIN file: dataloader reads `prompt`, OfflineTeacherRollout reads prompt->response.
OFFLINE_TEACHER_DATA_PATH=${OFFLINE_TEACHER_DATA_PATH:-"${OPV_ROOT}/examples/4b_offpolicy/teacher_sft/teacher_sft_all.parquet"}
OFFLINE_TEACHER_RESPONSE_KEY=${OFFLINE_TEACHER_RESPONSE_KEY:-response}

TRAIN_FILE=${TRAIN_FILE:-"${OFFLINE_TEACHER_DATA_PATH}"}

# Validation sets (for val_before_train + periodic evaluation) — AIME 2024/2025.
...
TEST_FILES=${TEST_FILES:-"['${AIME24_TEST_FILE}', '${AIME25_TEST_FILE}']"}
VAL_MAX_SAMPLES=${VAL_MAX_SAMPLES:--1}

MAX_POSITION_EMBEDDINGS=${MAX_POSITION_EMBEDDINGS:-32768}
ENABLE_THINKING=${ENABLE_THINKING:-false}   # Qwen3-4B teacher is non-thinking (math)
...
LOCAL_DIR_BASE=${LOCAL_DIR_BASE:-"${OPV_ROOT}/examples/4b_offline_distill"}
