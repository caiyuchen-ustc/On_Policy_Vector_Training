...
# Experiment defaults
# -----------------------------
PROJECT_NAME=${PROJECT_NAME:-Code_DAPO}
EXP_NAME_PREFIX=${EXP_NAME_PREFIX:-DAPO-Qwen3-8B-Code-RL}
...
# Code training data (Eurus) + validation (Eurus + LiveCodeBench v5).
# Validation defaults to the first-100-rows subsets for fast eval; override CODE_VAL_FILE /
# LCB_VAL_FILE with the full files (code_validation.parquet / livecodebench_v5.parquet) for a
# complete evaluation.
TRAIN_FILE=${TRAIN_FILE:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/G-OPD/data/G-OPD-Training-Data/Eurus/code_train.parquet"}
CODE_VAL_FILE=${CODE_VAL_FILE:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/G-OPD/data/G-OPD-Training-Data/Eurus/code_validation_100.parquet"}
LCB_VAL_FILE=${LCB_VAL_FILE:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/G-OPD/data/G-OPD-Training-Data/Eurus/livecodebench_v5_100.parquet"}
...
# Code prompts are short (Eurus train <=1002 tok, LCB v5 <=1876 tok), so 2048 is safe.
...
# -----------------------------
# Code sandbox (optional accelerator/isolator for the code reward)
# -----------------------------
# When SANDBOX_FUSION_URL is set, the code reward routes to sandbox_fusion: each test case is
# POSTed to the sandbox HTTP service and executed concurrently (off the training process),
...
#
# NOTE on concurrency: SANDBOX_MAX_CONCURRENT below is the CLIENT-side semaphore.
# Keep it aligned with SandboxFusion local.yaml's server-side max_concurrency to avoid queue buildup.
# SANDBOX_MEMORY_LIMIT_MB -> /run_code memory_limit_MB.
SANDBOX_FUSION_URL=${SANDBOX_FUSION_URL:-}
SANDBOX_MAX_CONCURRENT=${SANDBOX_MAX_CONCURRENT:-32}
SANDBOX_MEMORY_LIMIT_MB=${SANDBOX_MEMORY_LIMIT_MB:-4096}
# Per-case execution timeout (seconds). Default 2s.
SANDBOX_RUN_TIMEOUT=${SANDBOX_RUN_TIMEOUT:-1}
# Max test cases per problem during TRAINING reward scoring.
# Default 10: enough for reliable reward signal while cutting sandbox calls ~4x.
# Set to -1 to use all test cases.
SANDBOX_MAX_CASES=${SANDBOX_MAX_CASES:-5}
# Max test cases per problem during VALIDATION (test_freq).
# Default 3: validation only needs to track trend, not exact pass-rate.
# Much fewer calls → ~12x faster validation. Set to -1 to use all.
...
DEFAULT_LOCAL_DIR_BASE=${DEFAULT_LOCAL_DIR_BASE:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/GOPD/verl/examples/code_rl/repre_qwen3-4b-code-rl"}
...
SAVE_VECTOR_BASE_DIR=${SAVE_VECTOR_BASE_DIR:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/GOPD/verl/repre/code_rl/trainable_vectors"}
...
# Route the code reward through the sandbox service only when a URL is provided.
# These are existing config keys (reward_model.sandbox_fusion.*), so no leading '+'.
if [ -n "${SANDBOX_FUSION_URL}" ]; then
    echo "[code_rl] Using sandbox_fusion for code reward: ${SANDBOX_FUSION_URL} (run_timeout=${SANDBOX_RUN_TIMEOUT}s train_max_cases=${SANDBOX_MAX_CASES} val_max_cases=${SANDBOX_VAL_MAX_CASES})"
    REWARD_ARGS+=(
    ...
    echo "[code_rl] SANDBOX_FUSION_URL not set -> using in-process prime_code for code reward."
fi
