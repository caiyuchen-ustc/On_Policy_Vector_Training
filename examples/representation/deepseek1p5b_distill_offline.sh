...
# ============================================================================
# OFFLINE teacher-rollout reverse-KL distillation for DeepSeek-R1-Distill-Qwen-1.5B on SciKnowEval.
#
# Trains the student to match a frozen teacher (BroRL-1.5B) on PRE-GENERATED teacher rollouts
# (teacher_sft_all.parquet from gen_teacher_rollouts_deepseek1p5b.sh). No vLLM sampling happens:
...
#
# Three modes via the wrappers (deepseek1p5b_distill_offline_{fullparam,lora8,single}.sh):
...
PROJECT_NAME=${PROJECT_NAME:-DeepSeek1p5B_SciKnowEval_OfflineDistill}
EXP_NAME_PREFIX=${EXP_NAME_PREFIX:-DeepSeek-1.5B-SciKnowEval-OfflineDistill}

NNODES=${NNODES:-1}
NGPUS_PER_NODE=${NGPUS_PER_NODE:-8}

# Student (DeepSeek-R1-Distill-Qwen-1.5B) + teacher to distill from (BroRL-1.5B = ref model).
MODEL_PATH=${MODEL_PATH:-"${OPV_LEGACY_MODEL_ROOT}/DeepSeek-R1-Distill-Qwen-1.5B"}
TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-"${OPV_ROOT}/deepseek1p5b_brorl_rl/checkpoints/fullparam_lr1e-6/global_step_820/hf_model"}

# Offline teacher rollouts (prompt + response), produced by gen_teacher_rollouts_deepseek1p5b.sh.
# Doubles as the TRAIN file: dataloader reads `prompt`, OfflineTeacherRollout reads prompt->response.
OFFLINE_TEACHER_DATA_PATH=${OFFLINE_TEACHER_DATA_PATH:-"${OPV_ROOT}/examples/deepseek1p5b_offpolicy/teacher_sft/teacher_sft_all.parquet"}
OFFLINE_TEACHER_RESPONSE_KEY=${OFFLINE_TEACHER_RESPONSE_KEY:-response}

TRAIN_FILE=${TRAIN_FILE:-"${OFFLINE_TEACHER_DATA_PATH}"}

# Validation set (for val_before_train + periodic evaluation) — SciKnowEval.
DATA_ROOT=${DATA_ROOT:-"${OPV_LEGACY_DATA_ROOT}/sciknoweval"}
VAL_FILE=${VAL_FILE:-"${DATA_ROOT}/sciknoweval_validation.parquet"}
TEST_FILES=${TEST_FILES:-"['${VAL_FILE}']"}
VAL_MAX_SAMPLES=${VAL_MAX_SAMPLES:-100}

VERIFY_SCRIPT_PATH=${VERIFY_SCRIPT_PATH:-"${OPV_ROOT}/verl/utils/reward_score/sciknoweval_verify.py"}
MAX_POSITION_EMBEDDINGS=${MAX_POSITION_EMBEDDINGS:-32768}
ENABLE_THINKING=${ENABLE_THINKING:-true}   # DeepSeek-R1-Distill is a thinking model
...
LOCAL_DIR_BASE=${LOCAL_DIR_BASE:-"${OPV_ROOT}/examples/deepseek1p5b_offline_distill"}
