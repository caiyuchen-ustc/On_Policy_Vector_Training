...
# Single-vector RL wrapper, alternating positive/negative advantage samples per batch.
...
# ---- 交替正负 advantage: 偶数 step 只训 adv>0 样本, 奇数 step 只训 adv<0 样本 ----
# 注意: reward 是 0/1, 不存在 reward<0; 这里按 GRPO advantage 的正负来区分
# (adv>0 = 比组内平均好 -> 强化; adv<0 = 比平均差 -> 压低)
export ALTERNATE_ADV_SIGN=${ALTERNATE_ADV_SIGN:-true}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-deepseek1p5b_rl_singlev_altadvsign_layers${LAYER_TAG}_lr${ACTOR_LR}}
# RAY_DEBUG=legacy ray start --head --dashboard-host=0.0.0.0 --ray-debugger-external

# nohup sh /apdcephfs_zwfy3/share_302867165/ewencai/CODE/G-OPD/verl/examples/representation/deepseek1p5b_rl_single_altadvsign.sh >1p5b_rl_single_altadvsign.log 2>&1 &
