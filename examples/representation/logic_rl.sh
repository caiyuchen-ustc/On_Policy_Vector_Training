...
# ============================================================================
# Knights-and-Knaves (K&K) logic RL for G-OPD (the Logic-RL setup,
# arXiv:2502.14768).
#
# Reward = rule-based exact-match of every inhabitant's knight/knave role
# (verl/verl/utils/reward_score/kk_verify.py), wired through a custom reward
# function + the naive reward manager.
#
# Data is built by data/logic_kk/convert_kk.py from K-and-K/knights-and-knaves:
#   train = train/people{2..8}, validation = test/people{2..8} (capped to
#   VAL_MAX_SAMPLES rows at load time, default 100).
#
# Env-driven, three training modes (selected by the thin wrappers
# logic_rl_{fullparam,lora8,single}.sh):
...
# -----------------------------
# K&K reward knobs (passed through to kk_verify.py via the env)
# -----------------------------
export KK_REQUIRE_FORMAT=${KK_REQUIRE_FORMAT:-0}
export KK_VERBOSE=${KK_VERBOSE:-0}

# -----------------------------
# Experiment defaults
# -----------------------------
PROJECT_NAME=${PROJECT_NAME:-Logic_KK}
EXP_NAME_PREFIX=${EXP_NAME_PREFIX:-KK-Qwen3-8B-Logic-RL}
...
# K&K training data (train/people2-8) + validation (test/people2-8), built by
# data/logic_kk/convert_kk.py.
DATA_ROOT=${DATA_ROOT:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/G-OPD/data/logic_kk"}
TRAIN_FILE=${TRAIN_FILE:-"${DATA_ROOT}/kk_train.parquet"}
VAL_FILE=${VAL_FILE:-"${DATA_ROOT}/kk_validation.parquet"}
TEST_FILES=${TEST_FILES:-"['${VAL_FILE}']"}

# Cap the validation set at this many rows at load time (keeps validation fast).
# Set to -1 to use the full val file.
VAL_MAX_SAMPLES=${VAL_MAX_SAMPLES:-100}

VERIFY_SCRIPT_PATH=${VERIFY_SCRIPT_PATH:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/G-OPD/verl/verl/utils/reward_score/kk_verify.py"}
...
# K&K prompts are short; responses can be long (multi-step deduction).
...
DEFAULT_LOCAL_DIR_BASE=${DEFAULT_LOCAL_DIR_BASE:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/GOPD/verl/examples/logic_rl/repre_qwen3-4b-kk-rl"}
...
SAVE_VECTOR_BASE_DIR=${SAVE_VECTOR_BASE_DIR:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/GOPD/verl/repre/logic_rl/trainable_vectors"}
...
# K&K reward: rule-based exact-match, via the custom reward function + naive manager.
REWARD_ARGS=(
