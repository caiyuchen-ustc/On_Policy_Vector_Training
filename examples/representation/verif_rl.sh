...
# ============================================================================
# Instruction-Following (IF) RL, VerIF-style, for G-OPD.
#
# Reward = rule-based code checks x LLM soft-constraint judge (THU-KEG/VerIF,
# arXiv:2506.09942), wired through a custom reward function + the naive reward
# manager (NOT the dapo/overlong-buffer path used by code/science RL).
#
# Data is built by data/verif/convert_verif.py from THU-KEG/VerInstruct.
# The LLM judge is OFF by default (rule-only). Enable it by exporting
#   VERIF_LLM_ENABLE=1  IF_LLM_VERIFIER_URL=...  IF_LLM_VERIFIER_MODEL=...
# (see verl/verl/utils/reward_score/verif_verify.py for all knobs).
#
# This script is env-driven and supports three training modes, selected by the
# thin wrappers verif_rl_{fullparam,lora8,single}.sh:
...
# Network env (the verifier LLM endpoint, if used, is usually node-local so keep
# it out of the proxy; override outside if your endpoint is remote).
...
# Experiment defaults
# -----------------------------
PROJECT_NAME=${PROJECT_NAME:-IF_VerIF}
EXP_NAME_PREFIX=${EXP_NAME_PREFIX:-VerIF-Qwen3-4B-IF-RL}

NNODES=${NNODES:-1}
NGPUS_PER_NODE=${NGPUS_PER_NODE:-8}

MODEL_PATH=${MODEL_PATH:-"/apdcephfs_zwfy3/share_302867165/xxucaxu/models/raw/Qwen3-14B"}
...
DEFAULT_LOCAL_DIR_BASE=${DEFAULT_LOCAL_DIR_BASE:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/GOPD/verl/examples/verif_rl/repre_qwen3-4b-if-rl"}
...
SAVE_VECTOR_BASE_DIR=${SAVE_VECTOR_BASE_DIR:-"/apdcephfs_zwfy3/share_302867165/ewencai/CODE/GOPD/verl/repre/verif_rl/trainable_vectors"}
