...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # Training uses max_cases from config (sandbox_fusion.max_cases); validation
        # uses sandbox_fusion.val_max_cases if set, falling back to max_cases.
        # Keeping validation cheaper than training is fine: we only need trend accuracy.
        _sb_cfg = config.reward_model.get("sandbox_fusion") or {}
        _train_max_cases = _sb_cfg.get("max_cases", None)
        _val_max_cases_cfg = _sb_cfg.get("val_max_cases", None)
        # If val_max_cases not set explicitly, use the same as training.
        _val_max_cases = _val_max_cases_cfg if _val_max_cases_cfg is not None else _train_max_cases
        reward_fn = load_reward_manager(
            config, tokenizer, num_examine=0,
            max_cases_override=_train_max_cases,
            **config.reward_model.get("reward_kwargs", {}),
        )
        val_reward_fn = load_reward_manager(
            config, tokenizer, num_examine=1,
            max_cases_override=_val_max_cases,
            **config.reward_model.get("reward_kwargs", {}),
        )
