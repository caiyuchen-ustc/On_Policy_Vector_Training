...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            proxy = None
            if config:
                proxy = config["trainer"].get("wandb_proxy", None)

            # W&B uses multiple HTTP paths (GraphQL, file streaming, etc.).
            # Set both proxy fields plus the requests-style proxy map so the
            # SDK consistently routes all online traffic through the proxy.
            if proxy:
                settings = wandb.Settings(
                    http_proxy=proxy,
                    https_proxy=proxy,
                    x_proxies={"http": proxy, "https": proxy},
                )
...
class FileLogger:
    def __init__(self, project_name: str, experiment_name: str):
        ...
        # Resume must preserve previous measurements; the step field identifies duplicates.
